...
if (!empty($this->session->data['delivery_date'])) {
    $comment .= "\nДата доставки: " . $this->session->data['delivery_date'];
}
if (!empty($this->session->data['delivery_time'])) {
    $comment .= "\nВремя доставки: " . $this->session->data['delivery_time'];
}
if (!empty($this->session->data['need_call'])) {
    $comment .= "\nПредварительный звонок: Да";
}
